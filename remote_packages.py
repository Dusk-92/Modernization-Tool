...
def _migrate_legacy_pink_herbs_patch(target_dir, downloaded_path, progress=None):
    ...
    Pink Herbs used to be installed as Data\\patch-H.mpq. Other visual packs
    may also use that filename, so a user may have replaced the old managed
    file manually. During the v2 migration we only remove/restore patch-H when
    the live file is missing or exactly matches the freshly downloaded Pink
    Herbs MPQ. Otherwise the current patch-H is preserved and ownership is
    forgotten before Pink Herbs moves to patch-V.mpq.
    """
    ...
    data = _load_managed_manifest_data(target_dir, mod_id)
    remaining = [
        rel for rel in files if os.path.normcase(rel) != legacy_key
    ]
    _write_managed_manifest(
        target_dir,
        mod_id,
...
def install_pink_herbs(target_dir, progress=None):
    ...
    try:
        _verify_mpq(temp_path)
        _migrate_legacy_pink_herbs_patch(
            target_dir,
            temp_path,
            progress=progress,
        )
